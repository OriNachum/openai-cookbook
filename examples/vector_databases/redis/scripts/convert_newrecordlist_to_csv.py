...
class NewRecord:
    def __init__(self, question: str, company_name: str, company_size: str, company_location: str, company_industry: str,
                 conversation_date: date, model: str):
...
def convert_newrecordlist_to_csv(records: List[NewRecord], filename: str):
    ...
    with open(filename, mode='w', newline='') as file:
        ...
        writer.writerow(["id","question", "company_name", "company_size", "company_country", "company_industry", "conversation_date", "model"])  # Write the header
        for record in records:
            id=generate_unique_public_id()
            writer.writerow([id, record.question, record.companyName, record.companySize, record.companyCountry, record.companyIndustry, record.conversationDate, record.model])